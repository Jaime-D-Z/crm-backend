...
async function createUser(req, res) {
    ...
    try {
        ...
        // Process face photo if provided - WITH Face Recognition
        let faceDescriptor = null;
        let savedPhotoUrl = null;

        if (photo_url_base64) {
            try {
                // Initialize face-api if not loaded
                await faceApiService.initFaceApi();
                ...
                console.log(`✅ Foto y descriptor guardados: ${savedPhotoUrl}`);
            } catch (errorPhoto) {
                console.error("Error procesando foto:", errorPhoto);
                return res.status(400).json({ 
                    error: 'Error al procesar la imagen. Asegúrate de que la foto muestre claramente tu rostro.' 
                });
            }
        }
        ...
        // Update additional fields including face_descriptor
        await query(
            `UPDATE users SET phone=$1, department=$2, position=$3, is_active=$4, photo_url=$5, face_descriptor=$6 WHERE id=$7`,
            [phone || null, department || null, position || null, status === 'active', savedPhotoUrl, JSON.stringify(faceDescriptor), user.id]
        );
        await query(
...
async function updateUser(req, res) {
    try {
        ...
        if (faceDescriptor && JSON.stringify(faceDescriptor) !== user.face_descriptor) {
            updates.push(`face_descriptor = $${paramIndex++}`);
            params.push(JSON.stringify(faceDescriptor));
        }
